...
def main():
    ...
    tam = len(agentes)
    #
    # Leer el contenido del HTML y almacenarlo en la variable NuevoHTML 
    with open('CorreoIngresoPlataforma.txt', 'r', encoding='utf-8') as file:
        NuevoHTML = file.read()
    #    
    ...
    for index, drow in agentes.iterrows():
        ...
        print(
            "Gestionando agente para el curso ", str(drow["IDCURSO"]), " -> ",
            index + 1,
            "de",
            len(agentes),
            "(" + str(round(percentage,2)) + "%)",
        )
        classadd.CambioIngresoPlataforma(driver, drow, NuevoHTML)
