...
def crear_WebDriver():
    ...
    WD = Service("..\Chrome\chromedriver.exe")
...
def login(ldriver, usr, pwd, dfa):
    try:
        ...
        print ("<<Si pudo ingresar con las credenciales>> ✔️")
        return True

    except:
        print ("**No pudo ingresar con las credenciales** ❌")
        return False
...
def setdataAgen(ldriver, dlrow):
    ...
    agenteid="https://virtual.upb.edu.co/d2l/le/intelligentagents/agent/"+str(dlrow["IDCURSO"])+"/edit?agentId="+str(dlrow["IDAGENTE"])
    print(agenteid)
    ...
    #**Control de cambio*
    #checkBox = WebDriverWait(ldriver, 10).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "#detailsData\\$Status']"))) 
    #checkBox = WebDriverWait(ldriver, 10).until(EC.element_to_be_clickable((By.XPATH, "//input[@id='detailsData$Status']"))) #//cambio 2023/06/09
    #checkBox = WebDriverWait(ldriver, 10).until(EC.element_to_be_clickable((By.XPATH, "//*[@id='detailsData$Status']")))
    #checkBox = WebDriverWait(ldriver, 10).until(EC.element_to_be_clickable((By.XPATH, "(//input[@id='detailsData$Status'])[1]"))) #cambio 2024/04/30
    #**
    ...
    if str(checkBoxestado)=="None":
        ...
        #abrir planeacion colapsado este es *nuevo 2023/06/09*
        ...
        root1 = ldriver.execute_script('return arguments[0].shadowRoot', root1)

        root2 = root1.find_element(By.CLASS_NAME, "d2l-dropdown-opener")
        root2 = ldriver.execute_script('return arguments[0].shadowRoot', root2)
    
        ffin=root2.find_element(By.CSS_SELECTOR,"input[id^='d2l-uid']")

        #SE LIMPIA EL CAMPO DE FECHA FINAL Y INICIAL
        fini.clear()
        ffin.clear()

        # Convertir los objetos Timestamp a cadenas en el formato YYYY-MM-DD
    ...
    #button = ldriver.find_element(By.XPATH, '/html[1]/body[1]/div[8]/d2l-floating-buttons[1]/button[1]') #cambio 2023/10/30  ###(//button[normalize-space()='Save and Close'])[1]
    if language=="es-MX": 
       #button = ldriver.find_element(By.XPATH, '//button[text()="Guardar y cerrar"]') # cambio 2024/05/14
       button = WebDriverWait(ldriver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//button[contains(@class, 'd2l-button') and contains(text(), 'Guardar y Cerrar')]"))
       )
    else:
        #button = ldriver.find_element(By.XPATH, '//button[text()="Save and Close"]')  # cambio 2024/05/14
        button = WebDriverWait(ldriver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "//button[contains(@class, 'd2l-button') and contains(text(), 'Save and Close')]"))
        )

    #SE ACTIVA CLIC EN EL BOTON DE GUARDAR Y CERRAR
    button.click()
    sleep(5)
